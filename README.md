...
- **Push notifications** — In the user menu, click "Activar notificaciones" to allow browser push. One notification per day at **9:00** with pending doses (e.g. "9:00 Mañana: Pimobendan (Nilo). 21:00 Noche: …"). If you see "Toca para copiar la URL" when opening the app, that's Chrome's PWA message, not Meds; the real reminder is the one sent by the cron at 9:00. Requires VAPID keys and cron (see `.env.local.example`).
---
